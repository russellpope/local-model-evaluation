...
**That is exactly what happened.** *Proposed resolution:* the descriptor must carry raw,
vCenter-observable device/adapter attributes rather than a pre-decided protocol string, and the
production path must construct it from the datastore's actual backing storage — so that the
classifier's table test exercises a real decision instead of echoing its input.
