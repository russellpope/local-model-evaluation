...
// BindFlags declares command-line flags on a flag set.
// It does NOT bind flags to viper or set up environment variable bindings.
// Call this once per flag set (e.g. on PersistentFlags of the root command).
...
// Load reads configuration from flags, env vars, config file, and defaults.
// It declares flags on the flag set if they are not already declared,
// binds them to viper, and resolves values.
func Load(flags *pflag.FlagSet) (*Config, error) {
	...
	// Declare flags only once — skip if already declared (e.g. persistent flags)
	...
	// Bind flags to viper (safe to call multiple times on the same set)
	...
	// Resolve values with precedence: flag > env > file > default
...
// ParseBoolEnv parses a string as a boolean for environment variable values.
